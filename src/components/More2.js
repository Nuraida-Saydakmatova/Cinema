...
class Main extends Component {
    ...
    componentDidMount() {
        axios.get(`https://api.themoviedb.org/3/tv/${this.props.match.params.more2id}?api_key=1ea54fff90d26ae7dc1f5e21fe637664&language=ru-RU`)
            .then((response) => {
        ...
        return this.state.con? <Spinner className={'spiner'} animation="border" />:
            <>    
                <div style={{backgroundImage:`url(https://image.tmdb.org/t/p/original/${y.backdrop_path})`}} className='gl-img'>
                    ...
                    <h1 style={{position:'absolute', bottom:50, left:270, color:'white'}}>{y.name}</h1>
